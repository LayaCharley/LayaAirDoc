...
| <span style="display:inline-block;width:70px">参数</span> | 意义                                                         |
| --------------------------------------------------------- | ------------------------------------------------------------ |
| 允许缩放                                                  | 勾选后，使用这个字体的文本可以设置字体大小使字符图片缩放。如果这里不勾选，那么无论文本设置里字号为多少，位图字体都保持原样，不进行缩放 |
| 字号                                                      | 位图字体的大小。在勾选“允许缩放”后有效。建议与文本组件的”font size“属性保持一致，并且创建后可使用”font size“调整字号 |
| 行高                                                      | 如果为0，就是采用字号大小作为行高。如果大于0，就是实际文本的行间距（两个位图字体上边界之间的距离）。创建后可以在文本组件的”leading“属性中设置行高 |
