...
点击`打开`后，如图2-4所示，会有插件包含的内容列表展示，如果有我们不想要的功能，比如在Unity里直接预览这个功能不想要，也可以把相关默认的勾选去掉，但是我们建议全部导入（默认是全选）。
...
导入完成后，我们可以看到菜单栏多出一个`LayaAir3D`，如图2-5所示。此时说明，已经导入成功。
...
打开场景后，导出的场景并不具有环境光sh信息，需要找到在Scene3D层级下的烘焙开关。如图5-2所示，点击`烘培`，对当前场景的环境光sh数据进行预计算，
