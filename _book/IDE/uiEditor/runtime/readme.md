...
其次，在UI开发过程中，我们会使用Image，Box，Tab等等这些LayaAir 提供的UI组件来构成，对于一套相对复杂的UI界面来说，大量的UI组件，用自定义脚本的方式管理是不科学的，需要每个组件都拖拽到自定义属性中，繁琐而费力。因此我们建议开发者，使用UI继承类来管理UI组件，后面我们再说UI继承类和自定义脚本类的区别。**UI继承类设计的目的就是在场景或者是预制体的根节点创建，能对其内部所有的组件进行方便的管理，具有即时添加和删除关联组件的能力。**
...
Runtime 属性中需要我们来选择一个 Script 脚本，如动图2-2所示，新建并拖入一个 Script 脚本，实际上这个脚本就是 Runtime 类
...
上述操作之后，我们可以在 Project 中看到多了一个 `Script.generated.ts` 代码，点击这个代码，如图2-3所示
