...
> Date：2022-11-08    Author：charley
...
场景中添加的任一节点都可以创建动画，下述介绍以cube为例。首先在场景创建一个cube节点，**选中cube节点**后，就可以在编辑器下方的**Timeline面板**中看到“创建”按钮。如图1-1所示：
...
保存名称后，看到如图1-3所示界面，可以看到时间轴动画编辑面板、动画组件、状态机、动画文件，即代表动画创建成功。
...
首次为节点创建动画的时候，不仅会创建一个自己命名的动画文件，还会自动创建一个动画状态机文件，

状态机文件命名是由`动画名_动画节点名`组成。效果如图1-4所示。
