#  LayaAir Native应用构建基础说明

从LayaAir3.2版本开始， LayaAir Native支持各系统平台的自动打包，

而对于资深的开发者，如果更习惯使用传统的开发环境来打安装包，也保留了构建为原生包工程的方案。

## 一、构建发布

对于原生安装包的发布与构建，很多基础的通用配置，我们在本篇将Native的通用属性集中介绍。
...
| 名称   | 说明                                                         |
| ------ | ------------------------------------------------------------ |
| 应用ID | 应用的包名，这个正常情况下是不可见的。一般采用反域名命名规则（有利于分辨和避免与系统中已经有的APP冲突)。<br />例如 : com.layabox.runtime.demo  。包名必须是 xxx.yyy.zzz 的格式，至少要有两级，即xxx.yyy 。否则打包会失败。 |
|        |                                                              |
|        |                                                              |
|        |                                                              |
|        |                                                              |
