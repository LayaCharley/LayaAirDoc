...
引擎插件能力是游戏引擎(作为插件)单独在淘宝客户端进行缓存。当小游戏首次启动时，如果本地已经存在同类别游戏引擎插件，可直接复用或可通过增量下载的方式快速下载，从而提升启动速度。



### 5.1 创建引擎插件

开发者需要在[淘宝开放平台](https://open.taobao.com/)创建引擎插件，并创建Demo项目。

> 详细步骤查看淘宝官方[文档](https://open.taobao.com/v2/doc#/abilityToOpen?docType=1&docId=121197&treeId=804)。



### 5.2 使用引擎插件

在使用LayaAir-IDE发布淘宝小游戏时，如图5-1所示需要勾选“是否启用插件”选项。
...
按照第三节中的讲解发布后，按照图5-2的方式，使用淘宝开发者工具打开发布后的LayaAir项目。

![5-2](img/5-2.png)

（图5-2）

项目打开后，如图5-3所示，打开game.json文件，可以看到配置信息：

![5-3](img/5-3.png)

（图5-3）

其中，pluginVersion和pluginId都可以在[淘宝开放平台](https://open.taobao.com/)的控制台中看到。

`pluginVersion`：引擎插件版本号，上传时，写要上传的引擎插件的版本号，如图5-4所示。

`pluginId`：控制台中可以查看插件的appId。
...
配置完成后，点击“调试”按钮，使用真机扫码即可查看效果。

如果删除`path`，则会自动使用远程的插件，如图5-5所示。
