...
> LayaAirIDE也支持基于LayaAir-Unity资源导入插件导出的模型与网格的导入(.lh与.lm后缀)。本篇主要是指外部模型文件，如果有lm网格配置的需求，本篇文档也适用。

这几种后缀的模型拖拽到入到项目资源的assets目录内，即会自动进行转换导入。
...
例如，可以通过该处的设置调整模型到符合需求的物理尺寸大小。效果对比如图3所示：
...
如果勾选去掉，会在场景上感觉到被放大了100倍。效果对比如图4所示：
...
如果美术在模型制作的时候，就有摄像机(Camera)、光源(Light)、可见性(Visibility)属性设置，LayaAir3-IDE默认是不导入的，建议在LayaAir-IDE的场景中进行添加与设置。

当开发者存在使用模型的这些内置数据需求时，也可以通过勾选这些选项，如图5所示。通过点击**应用**重新导入进来。
...
当模型中存在Lod数据时，勾选该选项，会导出Lod的数据，并在使用该模型的时候自动创建一个Lod Group组件来设置Lod数据。添加后的效果如图6所示。
...
有一些属性的设置比较好理解，我们通过表格进行概要介绍。

| 中文属性名   | 英文属性名           | 属性说明                                                     |
| ------------ | -------------------- | ------------------------------------------------------------ |
| 动态读写     | read Write           | 当勾选动态读写后，允许开发者在运行时动态访问或修改模型的数据。<br />需要注意的是，启用`动态读写`会增加内存的使用。<br />所以，如果你的模型并不需要在运行时进行修改，那么请不要打开这个选项以节省内存。 |
...
在处理法线效果的能力方面，主要有三个相关属性构成，分别是**法线**、**法线计算模式**、**切线**，如图7-1所示：
...
**法线**属性有三个选项，**导入**`import`、**计算**`calculate`、**无**`none`。如图7-2所示。
...
**法线计算模式**有**非加权**unweight与**面积加权**areaWeight这两个选项。如图7-3所示。
...
切线用于定义模型表面上每个顶点的切线向量。分别为**无**`none`、**计算**`calculate`、**导入**`import`。如图7-4所示。
...
#### 3.4 生成光照贴图UV `generate Lightmap UVs`

默认的模型通常只有一个UV，勾选生成光照贴图UV，以及对子项配置，用于再生成一个UV1来支持光照贴图。如图8所示。
...
`填充`参数用于在图表的边缘周围添加额外的像素空间。在纹理贴图生成过程中，图表可能会被放置在纹理图集中，为了避免图表之间的采样受到邻近图表的影响，以及为了提供一些渲染上的余地，可以在图表的边缘周围添加一定数量的像素填充。

它的效果是扩大了每个图表的实际尺寸，使其在图集中占用更多的空间。这可以提供更好的纹理采样边界，减少图表之间的颜色混合，改善边缘渲染效果。

在使用时，可以根据需要调整填充的值，以权衡渲染质量和图集尺寸。合理的值通常会提供更好的渲染效果，但过度增加填充范围，会导致图集浪费更多的空间等负面影响。
...
勾选 `动画数据压缩`后，如图9所示。可以根据配置对动画数据的体积进行压缩。
...
此处用于设置模型材质的访问位置来源，共有两种选项。如图10所示。
