...
> 如果需要设置线性偏移，并且关节中的一方是静态刚体的时候，建议将马达关节添加给静态刚体，这样最终另一刚体会直接与静态刚体的线性偏移点重合，看上去更加直观。否则动态刚体的线性偏移点会与静态刚体锚点主动重合，导致线性偏移点在运行的时候与编辑时候位置不一致（不影响结果，都可以实现出同样的运行效果）。
...
> 需要注意的是，由于角度偏移涉及力与质量的计算。设置角度偏移的目标刚体，必须要添加碰撞体，否则角度偏移无效果。
...
![对比过小的力和正常的最大力设置效果](img/23-3.gif)
...
![对比过小的扭矩和正常的最大扭矩设置效果](img/23-4.gif)

（动图23-4）

#### 4.6.5 修正系数 `correctionFactor`

当马达关节连接的刚体发生了位置偏差时，修正系数将确定多快的速度来纠正这一偏差，使得物体回到预期的相对位置。
...
![不产生振荡的系数和产生振荡的系数设置对比](img/23-5.gif)
