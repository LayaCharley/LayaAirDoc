...
当物体运动速度非常高时，传统的碰撞检测可能无法准确捕捉到碰撞的发生，导致互相穿透卡在一起。因为在两个时间步之间，物体可能已经移动了一个较大的距离。这种情况下子弹属性的启用变得十分重要。

如动图8所示，启用子弹属性后，物理引擎采用更为精确的碰撞检测算法，即使在高速运动的情况下，也能够准确地检测到碰撞。
...
刚体标签的默认值是RigidBody。如图12所示。

![](img/12.png) 

(图12)

刚体标签主要用于在代码中查找识别该刚体，当我们想在项目代码中，通过自定义的标签名来识别该刚体，用于判断控制的时候，可以在该属性中进行设置。
