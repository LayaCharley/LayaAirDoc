...
有时候，我们需要根据物理的碰撞事件在代码中实现逻辑交互。为此，LayaAir引擎在物理碰撞的首次发生时（`onTriggerEnter`）、持续碰撞时（`onTriggerStay`）以及退出碰撞时（`onTriggerExit`），均会触发这些特定的物理事件方法。这些方法在引擎中默认不包含具体实现，可以视为待开发者重写的方法。开发者只需继承`Laya.Script`类，并在其中重写这些方法，就可以实现自定义的逻辑响应，替代默认的空实现。

这样，通过重写这些特定方法，开发者可以根据物理碰撞的具体阶段执行相应的游戏逻辑或者交互效果，从而使得游戏或应用能够在遇到物理碰撞时，有更自然、更真实的响应。本小节就针对这些物理事件虚方法进行简单介绍，并提供给基础的使用示例，提供给开发者作为参照。

### 5.1 单次碰撞开始onTriggerEnter

`onTriggerEnter`是在每一次发生物理碰撞的开始时，引擎都会调用一次的事件方法。示例代码如下：

```typescript
class DemoScript extends Laya.Script {
    
    /**
     * 在每一次发生物理碰撞的开始时，引擎都会调用一次的事件方法。
     * @param other 碰撞目标对象的碰撞体以及所属节点对象等信息
     * @param self  自身的碰撞体以及所属节点对象等信息
     * @param contact  物理引擎携带的碰撞信息b2Contact，开发者可以通过查询b2Contact对象来获取两个刚体碰撞有关的详细信息。但是通常用不上，other和self中已存在常规需要的信用，足够用了
     */
    onTriggerEnter(other: Laya.ColliderBase, self?: Laya.ColliderBase): void {
        // 假如碰到了炸弹
        if (other.label == "bomb") {
            // 此处省略爆炸伤害的逻辑
...
### 5.2 单次碰撞持续发生onTriggerStay

`onTriggerStay`是发生持续的物理碰撞时，也就是碰撞生命周期内的第二次碰撞到碰撞结束前，每帧都在触发调用的事件方法。示例代码如下：

```typescript
class DemoScript extends Laya.Script {

    /**
    * 发生持续的物理碰撞时，也就是碰撞生命周期内的第二次碰撞到碰撞结束前，每帧都在触发调用的事件方法。
    * 尽量不要在该事件方法中执行复杂的逻辑和函数调用，尤其是运算等消耗性能的代码，否则会对性能有明显的影响。
    * @param other 碰撞目标对象的碰撞体以及所属节点对象等信息
    * @param self  自身的碰撞体以及所属节点对象等信息
    * @param contact  物理引擎携带的碰撞信息b2Contact，开发者可以通过查询b2Contact对象来获取两个刚体碰撞有关的详细信息。但是通常用不上，other和self中已存在常规需要的信用，足够用了
    */
    onTriggerStay(other: Laya.ColliderBase, self?: Laya.ColliderBase): void {
        //模拟角色进入毒气区域，产生持续伤害
        if (other.label == "poison") {
            // 此处省略毒气伤害的逻辑

            console.log("进入毒气区域：" + self.label + "受到持续伤害，生命值-1");
        }
...
> 提醒：尽管示例中区域检测逻辑使用了onTriggerStay，其实可以采用其它更高效的方法来替代，示例仅仅是说明了一种使用的可能性，在实际开发过程中，onTriggerStay能不使用尽量不要使用，该事件如果使用不当对性能的消耗会影响较大。

### 5.3 单次碰撞结束onTriggerExit

`onTriggerExit`是在每一次的物理碰撞结束时，引擎都会调用一次的事件方法。示例代码如下：
...
class DemoScript extends Laya.Script {
    ...
    * @param contact  物理引擎携带的碰撞信息b2Contact，开发者可以通过查询b2Contact对象来获取两个刚体碰撞有关的详细信息。但是通常用不上，other和self中已存在常规需要的信用，足够用了
    */
    onTriggerExit(other: Laya.ColliderBase, self?: Laya.ColliderBase): void {
        //模拟角色离开毒气区域，触发逃脱奖励
        if (other.label == "poison") {
            // 此处省略逃脱奖励的逻辑
