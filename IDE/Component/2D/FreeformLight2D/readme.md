...
**2D自由形态光（FreeformLight2D）** 是一种支持自定义光源形状的2D灯光类型。通过设置多个顶点位置，开发者可以自由定义光源的几何形态，从而实现非规则的光照效果。这种灯光类型非常适合表现复杂或不规则的光影场景，提供了极大的灵活性和创作空间。

### 应用场景

1. **非规则光源**
   适用于模拟非标准形状的光源，如裂缝中透出的光线、不规则的火焰光芒等。
2. **动态光影效果**
