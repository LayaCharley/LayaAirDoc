...
但是从LayaAir3.0开始，为了简化开发者的理解与流程，项目的入口不再是以入口类的形式出现。入口改为基于**启动场景**上挂载的自定义组件脚本或UI运行时。

而从3.2.0正式版开始，为了让开发者更灵活的控制游戏启动入口，**增加了启动脚本的设置，允许开发者自定义设置自己的游戏启动入口**。

也就是说，引擎的初始化配置工作仅需要在IDE里可视化的配置，即可完成。下文会详细介绍。



### 1.2 设置启动场景

LayaAir 3.x引擎，只能在IDE里设置启动场景作为项目的入口。如果需要初始化引擎全局配置，直接在IDE的项目设置中配置。如果需要代码逻辑，直接为场景关联脚本，即可以执行代码逻辑。
...
在构建发布面板中设置了启动场景，并且在预览运行入口处选择了启动场景后，开发者可以设置**启动脚本**。它可以自定义启动入口，用来**替代启动场景**，并且自行决定场景启动前要处理的逻辑，以及打开场景的时机。

![1-7](images/1-7.png)

（图1-7）

这个脚本需要导出一个名为main的函数。引擎初始化后将直接调用这个函数，不再打开默认场景，由开发者在入口函数中自行决定打开某个场景。代码示例如下：
...
如果开发者不使用启动脚本作为入口，仍然使用启动场景作为入口也是可以的，可以按照后面第二节的介绍设置脚本即可。



## 二、入口的逻辑脚本

从正常的流程上，代码的逻辑必须要跟随入口场景，通过入口场景激活与添加到舞台等引擎的生命周期方法来执行对应的逻辑。
